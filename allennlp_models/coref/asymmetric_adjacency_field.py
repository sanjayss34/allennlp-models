...
class AsymmetricAdjacencyField(Field[torch.Tensor]):
    """
    ...
    def __init__(
        ...
        if not all(
            0 <= index[1] < num_columns and 0 <= index[0] < num_rows for index in indices
        ):
    ...
    def get_padding_lengths(self) -> Dict[str, int]:
        return {"num_rows": self.sequence_field1.sequence_length(), "num_columns": self.sequence_field2.sequence_length()}

    @overrides
    def as_tensor(self, padding_lengths: Dict[str, int]) -> torch.Tensor:
        desired_num_rows = padding_lengths["num_rows"]
        desired_num_columns = padding_lengths["num_columns"]
        tensor = torch.ones(desired_num_rows, desired_num_columns, dtype=torch.int64) * self._padding_value
    ...
    def empty_field(self) -> "AdjacencyField":
        ...
        adjacency_field = AdjacencyField(
            empty_list, self.sequence_field1.empty_field(), self.sequence_field2.empty_field(), padding_value=self._padding_value
        )
        return adjacency_field

    def __str__(self) -> str:
        num_rows = self.sequence_field1.sequence_length()
        num_columns = self.sequence_field2.sequence_length()
        ...
        return (
            f"AdjacencyField of {num_rows} rows and {num_columns} columns\n"
            f"\t\twith indices:\n {formatted_indices}\n"
            f"\t\tand labels:\n {formatted_labels} \t\tin namespace: '{self._label_namespace}'."
        )

    def __len__(self):
        return len(self.sequence_field1)
